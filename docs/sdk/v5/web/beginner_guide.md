...
| [Create a user](login.html#register-a-user) <br/><br/>[Implement token retrieval](/rest/easemob_app_token.html)    | - **Create a user**: You can [call a REST API to create a user](/rest/account_register_open.html) or create one in the [EasyIM Console](https://console.easyim.ai/user/login). For details, see [Register a user](login.html#register-a-user).<br/> - **Obtain a token**: Integrate the [app token retrieval API](/rest/easemob_app_token.html) and [user token retrieval API](/rest/easemob_user_token.html) into your app server to implement token retrieval. Your app can call its own server to obtain a token from the EasyIM server.   |
