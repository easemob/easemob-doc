...
| Step            | Description | 
| :-------------- | :----- | 
| [Register an account](/product/console/account_register.html#注册账号)  | Before using Easemob products, first go to the [Easemob Console](https://console.easemob.com/user/login) to [register an account](/product/console/account_register.html#注册账号).<br/>Your Easemob account uniquely identifies you as a developer in the Easemob Console. Use the account to log in to the console and configure and manage apps. When integrating your app with EasyIM, map the EasyIM accounts to the accounts in your app. |
| [Create an app](/product/console/app_create.html)<br/><br/>[Obtain the App Key](/product/console/app_manage.html#获取应用凭证) | 1. To integrate EasyIM services, first [create an app](/product/console/app_create.html) in the Easemob Console.<br/> 2. Easemob assigns an App Key to the app as its unique identifier. [Obtain the App Key](/product/console/app_manage.html#获取应用凭证) and pass it when integrating the SDK.  |
...
| [Create a user](/document/web/login.html#register-a-user) <br/><br/>[Implement token retrieval](/document/server-side/easemob_app_token.html)    | - **Create a user**: You can [call a REST API to create a user](/document/server-side/account_register_open.html) or create one in the [Easemob Console](https://console.easemob.com/user/login). For details, see [Register a user](login.html#register-a-user).<br/> - **Obtain a token**: Integrate the [Obtain an App Token API](/document/server-side/easemob_app_token.html) and [Obtain a User Token API](/document/server-side/easemob_user_token.html) into your app server to implement token retrieval. Your app can call its own server to obtain a token from the EasyIM server.   |
