# Configure Push Notification Modes and Do Not Disturb

To improve the user experience when handling large numbers of push notifications, the SDK provides fine-grained configuration of push notification modes and Do Not Disturb at the global and conversation levels through `client.pushManager`. You can centrally control mobile offline push based on the push notification mode, a specified duration, or a daily period.

## Feature activation

[Push notification modes](push_notification_mode_dnd.html#push-notification-modes) and [Do Not Disturb](push_notification_mode_dnd.html#do-not-disturb) are advanced push features. Before using them, enable them for free in the [EasyIM Console](https://console.easemob.com/user/login). **After activation, to disable advanced push features, you must contact the EasyIM business manager because this operation deletes all configurations related to the advanced features.**
...
After initializing and successfully logging in to the SDK, configure Do Not Disturb globally at the app level or for a specified conversation. While Do Not Disturb is in effect, EasyIM does not send push notifications to offline users in the corresponding scope.

Do Not Disturb supports the following rule types:

- `DURATION` (one-time Do Not Disturb): Takes effect immediately after configuration and automatically ends when it expires. It is suitable when the user temporarily does not want to be disturbed.
- `INTERVAL` (daily recurring Do Not Disturb): Configures a period that recurs daily, such as `23:00` to `07:00` the following day. It is suitable for a regular rest period.

The following table describes the Do Not Disturb time parameters:

| Rule mode | Parameter | Type | Description | Scope |
| :--- | :--- | :--- | :--- | :--- |
| `INTERVAL` | `startTime` and `endTime` | Object | Daily recurring Do Not Disturb period, using the 24-hour clock with minute-level precision in the format `{ hours, minutes }`. Value ranges: 0–23 for hours and 0–59 for minutes.<br/> - **Daily scheduled trigger**: After configuration, Do Not Disturb starts automatically during the specified period each day.<br/> - **Cross-day support**: If the end time is earlier than the start time, the period crosses midnight and continues from the start time on the current day to the end time on the next day. For example, 10:00–8:00 means that Do Not Disturb applies from 10:00 on the current day to 8:00 on the next day.<br/> - **All day and disable**: If the start and end times are the same, Do Not Disturb applies all day. Set 0:00–0:00 to disable Do Not Disturb.<br/> - **Single-period limit**: Only one daily Do Not Disturb period is supported. A new configuration overwrites the old configuration.<br/> - **Effective time**: The configuration takes effect immediately. For example, if 8:00–12:00 is configured at 11:00, it takes effect from 11:00 to 12:00 that day and then applies from 8:00 to 12:00 each day. | App-wide only. |
| `DURATION` | `duration` | Int | One-time Do Not Disturb duration in minutes. The value range is `0`–`10080`, or 0 to 7 days, where `0` means that the parameter has no effect.<br/> - **Effective once**: Unlike a daily recurring Do Not Disturb period, this parameter applies once and begins counting down immediately after configuration.<br/> - **Example**: If `duration = 240` (4 hours) is configured at 8:00 AM, the app is in Do Not Disturb from 8:00 AM to 12:00 PM that day. | App-wide or for a specified one-to-one or group conversation. |
...
- Starting the next day, only the Do Not Disturb period recurs daily. The Do Not Disturb duration is not triggered again.

**Example**: At 8:00 AM, set `startTime/endTime` to 8:00–10:00 and `duration = 240` (4 hours). The result is:

- **Current day**: Do Not Disturb applies from 8:00 AM to 12:00 PM. The period setting covers 8:00–10:00, and the duration setting covers 10:00–12:00.
- **Starting the next day**: Do Not Disturb applies from 8:00 AM to 10:00 AM each day. Only the period setting applies.

**Relationship between push notification modes and Do Not Disturb**

Do Not Disturb has a higher priority than the push notification mode. For example, if a conversation's push notification mode is `ALL`, but the conversation is currently within a Do Not Disturb period or duration, you still do not receive offline push notifications for the conversation while Do Not Disturb is in effect.

If one-time Do Not Disturb is configured only for a conversation and Do Not Disturb is not configured globally for the app, offline push notifications are not sent for that conversation while Do Not Disturb is in effect. Other conversations continue sending push notifications according to their own push notification modes or the inherited global setting.
...
- `DURATION`: Sets a one-time Do Not Disturb duration.
- `INTERVAL`: Sets a daily recurring Do Not Disturb period.
...
await client.pushManager.setGlobalSilentMode({
  rule: {
    mode: 'DURATION',
    duration: 7200, // One-time Do Not Disturb duration.
  },
});

await client.pushManager.setGlobalSilentMode({
  rule: {
    mode: 'INTERVAL', // Daily recurring Do Not Disturb period
    startTime: {
      hours: 8,
      minutes: 0,
    },
...
## Retrieve global push-receiving rules
...
Call `client.pushManager.setConversationSilentMode` to set the offline-push notification receiving rules for a specified conversation, including the push notification mode, Do Not Disturb duration, or Do Not Disturb period, as shown in the following example.
...
// Set Do Not Disturb for a conversation
await client.pushManager.setConversationSilentMode({
  conversationId: '12345567',
  conversationType: 'groupChat',
  rule: {
    mode: 'DURATION', // Do Not Disturb duration.
    duration: 7200, // Do Not Disturb duration in milliseconds.
  },
});
