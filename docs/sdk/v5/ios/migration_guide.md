...
| `EMStreamChunk.sequenceNumber` | `EMStreamChunk.isComplete` | Determines the stream message chunk status. | Version 5.0.0 no longer exposes the chunk sequence number. Use the read-only property to determine whether the stream message is complete. |
