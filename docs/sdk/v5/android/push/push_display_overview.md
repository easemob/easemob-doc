...
1. [**Set notification display attributes**](push_display_attribute.html): Use the default push title and content, that is, the push notification display mode `DisplayStyle` uses the default value (`SimpleBanner`). The push title is **You have a new message**, and the push content is **Please tap to view**.
