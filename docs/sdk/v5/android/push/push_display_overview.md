...
1. [**Set notification display attributes**](push_display_attribute.html): Use the default push title and content, that is, the push notification display mode `DisplayStyle` uses the default value (`SimpleBanner`). The push title is **You've got a new message**, and the push content is **Please click to view**.
...
4. [**The recipient sets a push template**](push_template.html#the-message-recipient-uses-a-push-template).
