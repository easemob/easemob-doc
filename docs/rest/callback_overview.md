...
Before using webhooks, check whether your plan supports this feature. For details, see [Value-added service fees](/product/pricing_policy.html#value-added-service-fees). If your plan does not support webhooks, first open the [EasyIM Console](https://console.easemob.com/user/login) to [activate the feature](/product/console/basic_webhook.html#message-callback-1), and then [configure webhook rules](/product/console/basic_webhook.html#configure-message-callback-rules).
...
1. Locate the following information:
   ...
   - The secret generated by the EasyIM server for the webhook rule: You can find it in the webhook rule list on the **Message webhook** page of the [EasyIM Console](https://console.easemob.com/user/login).
