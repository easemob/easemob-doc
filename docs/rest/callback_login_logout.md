...
6. The user is forced offline from a [single device](/rest/account_offline_device_single.html) or [all logged-in devices](/rest/account_offline_forced.html). This operation can be initiated from the [client](/sdk/v5/android/multi_device.html#forcibly-log-out-a-specified-account-from-one-device), [server](/rest/account_offline_device_single.html), or [EasyIM Console](/product/console/operation_user.html#force-a-user-offline).
7. In a single-device login scenario, a device that logs in later forces the previously logged-in device offline.
8. When the maximum number of devices for [multi-device login](/sdk/v5/android/multi_device.html) is reached, a newly logged-in device forces a previously logged-in device offline. For multi-device login, EasyIM supports up to 4 simultaneously online devices per platform by default.
