...
6. The user is forced offline from a [single device](/rest/account_offline_device_single.html) or [all logged-in devices](/rest/account_offline_forced.html). This operation can be initiated from the [client](/document/android/multi_device.html#forcibly-log-out-a-specified-account-from-one-device), [server](/rest/account_offline_device_single.html), or [EasyIM Console](/product/console/operation_user.html#force-a-user-offline).
