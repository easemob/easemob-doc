# Chat Group and Chat Room Member Leave Callback

## Feature overview

When a member leaves a chat group or chat room voluntarily, is removed, or leaves after being added to the blocklist, the EasyIM server sends a callback request to your app server according to the [post-delivery callback rules](/product/console/basic_webhook.html#configure-message-callback-rules). Your app server can use the callback to identify the member who left and synchronize data.
