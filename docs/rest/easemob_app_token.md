...
| `client_id`     | String | Yes       | App's `client_id`, used to generate an app token for calling REST APIs. See the **App Overview** page in the [Easemob Console](https://console.easemob.com/user/login/).   |
| `client_secret` | String | Yes       | App's `client_secret`, used to generate an app token for calling REST APIs. See the **App Overview** page in the [Easemob Console](https://console.easemob.com/user/login/).     |
| `ttl`           | Long   | No       | Token validity period, in seconds.<br/> - If you pass this parameter, the token validity period is the specified value.<br/> - If you do not pass this parameter, the token validity period is based on the setting on the **User Management** page of the [Easemob Console](https://console.easemob.com/user/login/).<br/> - If set to `0`, the token never expires. <br/>Note: On a VIP 5 cluster, this parameter is measured in milliseconds.|
...
| :---------- | :-------- | :-------------- | :------------ | :----|
| 400         | illegal_argument    | client_id must be provided.        | `client_id` is not passed in the request body.| See the **Client ID** parameter corresponding to the App Key on the **App Overview** page in the [Easemob Console](https://console.easemob.com/user/login/). |
| 400         | illegal_argument                   | client_secret must be provided    | `client_secret` is not passed in the request body. | See the **Client Secret** parameter corresponding to the App Key on the **App Overview** page in the [Easemob Console](https://console.easemob.com/user/login/). |
| 400         | invalid_grant                      | client_id does not match   | The `client_id` corresponding to the App Key does not match the `client_id` passed in the request body. | See the **Client ID** parameter corresponding to the App Key on the **App Overview** page in the [Easemob Console](https://console.easemob.com/user/login/), and ensure that it matches the value passed in the request body. |
