...
| `maxusers`    | Int    | No       | Maximum number of chat room members, including the chat room owner. The value range is [1, 10,000], and the default value is `1000`. To adjust this limit, contact the Easemob business team. |
...
1. Before using this feature, you must **contact the Easemob business team**. After the feature is activated, you can also [create a chat group with a custom group ID](group_create.html#request-body-fields).
...
| 403     | exceed_limit | appKey:XX#XX has create too many chatrooms! | The appKey has reached the limit on the number of chat rooms it can create. | Delete unused chat rooms or contact the Easemob business team to adjust the limit. |
| 403     | exceed_limit | user XX has joined too many chatrooms! | The user has reached the limit on the number of chat rooms they can join. | Leave unused chat rooms or contact the Easemob business team to adjust the limit. |
| 403     | exceed_limit | members size is greater than max user size ! | The number of users added when creating the chat room exceeds the maximum (value range: [1,10,000]). | Contact the Easemob business team to increase the limit. |
