...
After a Reaction is added to or removed from a message, the EasyIM server sends a callback request to your app server according to the [post-delivery callback rules](/product/console/basic_webhook.html#configure-message-callback-rules). Your app server can use the callback to obtain Reaction information and synchronize data.
...
In a Reaction callback request, the `reactions` array contains all current Reaction records for the message. Each record has the following structure:
...
In the following callback request, the message with ID `99XXXX32` currently has two Reaction records: `test`, added by `user2`, and `test-1`, also added by `user2`. For `test-1`, the `op` field indicates that this Reaction was created in the current operation (`reactionType: "create"`) by `user2`.
...
|:---------------|:-------|:---------------|
| `chat_type` | String | The value is fixed as `notify`. Notification callbacks include callbacks for message threads and Reactions. Use `payload` and its `type` field to determine the specific type.|
...
| `msg_id`          | String | Message ID of the callback request.       |
