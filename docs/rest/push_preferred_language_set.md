# Set the Preferred Language for Push Notifications

## Feature overview

Sets the preferred language for offline push messages.

## Call frequency limit

100 calls/second/App Key

## Request URL

```http
PUT https://{host}/{org}/{app}/users/{userId}/notification/language
```

| Parameter       | Type   | Description          | Required |
| :--------- | :----- | :--------------------------------- | :------- |
| `userId` | String | User ID of the user whose preferred language for push notifications to set.  | Yes       | 

For details about the parameters in the request URL, see [Request URL parameters](overview.html#request-url).

## Request example

```bash
curl -X PUT 'https://XXXX/XXXX/XXXX/users/XXXX/notification/language' \
-H 'Authorization: Bearer <YourAppToken>' \
-H 'Content-Type: application/json' \
-d '{
    "translationLanguage":"EU"
}'
```

## Request header fields

For details about the `Content-Type`, `Accept`, and `Authorization` fields, see [Request header fields](overview.html#request-header-fields).

## Request body fields

| Parameter                  | Type   | Description                        | Required |
| :-------------------- | :----- | :--- | :------- |
| `translationLanguage` | String | Code of the user's preferred language for receiving push notifications. An empty string indicates that no translation is required, and the server directly pushes the notification in its original language. | Yes       |

## Response example

```json
{
  "path": "/users",
  "uri": "https://XXXX/XXXX/XXXX/users/XXXX/notification/language",
  "timestamp": 1648089630244,
  "organization": "hx",
  "application": "17fe201b-XXXX-XXXX-XXXX-1ed1ebd7b227",
  "action": "put",
  "data": {
    "language": "EU"
  },
  "duration": 66,
  "applicationName": "hxdemo"
}
```

## Response body fields

If the returned HTTP status code is 200, the request is successful. The response body contains the following fields:

| Field            | Type   | Description                               |
| :-------------- | :----- | :--------------------------------- |
| `data`          | JSON   | User's preferred language for receiving push notifications.       |
| `data.language` | String | Code of the user's preferred language for receiving push notifications. |

The other fields in the response body are described below:

| Field | Type | Description |
| :------------- | :----- | :---------------------- |
| `path`            | String | Request path, which is part of the request URL. You do not need to pay attention to it.       |
| `uri`             | String | Request URL. |
| `timestamp`       | Long   | Unix timestamp in milliseconds. |
| `organization`    | String | Unique identifier assigned by EasyIM to each company (organization), identical to the `org_name` request parameter. |
| `application`     | String | Unique identifier of the app in the system. It is generated by the system and does not require your attention. |
| `action`          | String | Request method. |
| `duration`        | Int    | Duration from sending the request to receiving the response, in milliseconds. |
| `applicationName` | String | App name entered when you created the app in the Easemob Console, identical to the `app_name` request parameter. |

## Error code

If the returned HTTP status code is not 200, the request fails. See [Common error codes](push_error.html) for possible causes.
