...
| `maxusers`            | Int    | No       | Maximum number of chat group members, including the group owner. The default is `200`. If the value exceeds `3000`, offline push is no longer supported by default. To enable offline push, contact the Easemob business team for activation.|
...
1. Before using this feature, **contact the Easemob business team** for activation. After activation, you can also [create a chat room with a custom chat room ID](chatroom_create.html#request-body-fields).
...
| 403     | exceed_limit | appKey:XX#XX has create too many groups! | The appKey has reached its chat group limit. | Delete unused chat groups or contact the Easemob business team to adjust the limit. For details, see [IM Package Function Details](/product/product_package_feature.html). |
