# Authenticate with an EasyIM User Token
...
| `ttl`        | Long   | No       | User token validity period, in seconds. Set it to `0` so the token never expires. If this parameter is not passed, the default validity period is 60 days. You can also set it on the **User Management** page of the [Easemob Console](https://console.easemob.com/user/login/). The most recent setting takes precedence. |
...
| 429         | resource_limited    | You have exceeded the limit of the community edition,Please upgrade to the enterprise edition | If `grant_type` in the request body is `inherit` and `autoCreateUser` is `true`, meaning the user is automatically registered when they do not exist, the number of registered users exceeds the plan limit. | The free plan supports up to 100 registered users. You can [upgrade to a paid plan](/product/pricing_method.html#subscribe-and-upgrade-a-plan). The Pro and Enterprise plans have no limit on the number of registered users. |
...
1. Create an app in the [Easemob Console](https://console.easemob.com/user/login) to generate `AppKey`, `Client ID`, and `ClientSecret`.
