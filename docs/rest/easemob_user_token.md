...
| 429         | resource_limited    | You have exceeded the limit of the community edition,Please upgrade to the enterprise edition | If `grant_type` in the request body is `inherit` and `autoCreateUser` is `true`, meaning the user is automatically registered when they do not exist, the number of registered users exceeds the plan limit. | The free plan supports up to 100 registered users. You can [upgrade to a paid plan](/product/pricing_method.html#subscribe-to-or-upgrade-a-plan). The Pro and Enterprise plans have no limit on the number of registered users. |
