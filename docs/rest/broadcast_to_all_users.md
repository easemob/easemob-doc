...
2. The API can be called 3 times per day. Contact the business manager to increase this limit. Exceeding it returns the 403 error "broadcast message limit exceeded".
...
| 403      | forbidden_op | message broadcast service is unopened  | The broadcast messaging feature is not activated. | Contact the business manager for activation. |
...
| 403      | forbidden_op | broadcast message limit exceeded  | The daily API call limit, 3 calls by default, has been exceeded. | Contact the business manager to increase the API call frequency limit. |
