...
- Changed the return value of `CursorResult#cursor` from `undefined` to an empty string (`""`) when the final page of roaming messages is retrieved via `ChatManager#asyncFetchHistoryMessages`.
- Removed the reflection implementation used for obtaining absolute file paths from `FileProvider`.
