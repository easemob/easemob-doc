...
目录索引位于好友或成员列表最右侧，为快速通过字母定位到相关好友的滚动条。

#### 2.13.2.列表索引(IndexInList)
列表内的索引用于不同字母分类的好友的分割。可以配置是否有底部分割线。
...
会话列表项为会话详情的入口，可展示的信息有:好友头像(Avatar)、好友昵称(Title)、最新消息（Subtitle）、新消息通知（Badge）、新消息时间戳（Time）。
样式上支持头像配置大/小圆角、是否配置列表项的分割线。

#### 3.5.3.好友列表项(ContactsItem)
好友列表项为好友详情的入口，可展示的信息有:好友头像(Avatar)、好友昵称(Title)、好友状态（Subtitle）。
样式上支持头像配置大/小圆角、是否配置列表项的分割线。
...
### 4.4.好友消息(ContactMsg)
好友消息为一种展示好友的气泡样式，支持点击事件，
支持显示的字段有：好友头像、好友昵称（Title）
...
#### 5.1.2.新会话(NewMsg)
通过会话右上角action调取的操作列表，可以此发起会话、创建群、添加好友。
##### 5.1.2.1. 发起会话(StartConversation)
点击此项将调取好友列表，终端用户可点选或者搜索好友，点击后将进入会话详情页面，用户可以通过发送消息发起会话。
##### 5.1.2.2. 创建群组(CreateGroup)
点击此项将调取多选状态的好友列表，终端用户在此页面点选群组成员，当群成员≥2(包含群主和群成员)时，用户可以点击创建进入群组会话详情页面，以创建一个群组。
##### 5.1.2.3. 添加好友(Addcontact)
点击此项将大家添加好友弹窗，终端用户输入要添加的好友ID后发送添加信息。

### 5.2.好友视图(ContactsView)
好友视图是好友搜索、好友列表项组合而成的页面。

#### 5.2.2.新申请列表(NewRequest)
如用户收到添加好友的申请，将在此展示相关信息。用户可以通过添加操作处理好友添加请求。

#### 5.2.3.群组列表(GroupList)
将在此展示用户加入的所有群。用户可以通过添加操作处理好友添加请求。
...
1)会话关键信息的展示：好友/群的头像、好友昵称/群名称、好友的在/离线状态等

2)关于相关会话的操作：退出会话、查看好友/群详情、针对当前会话的 1-3 个操作
...
### 5.4.好友详情视图（ContactDetailView）
好友详情为好友详情展示的页面，主要展示好友头像、昵称、ID等好友信息；
支持在本页面操作好友免打扰、好友删除操作；
支持在本页面进入消息详情等入口；
