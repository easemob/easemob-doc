...
对于底部输入栏，可通过覆盖同名资源文件的方式在 `Bundle.main` 中修改底部输入框的图标、图片和国际化资源。

### 常用图标替换

对于底部输入栏 `MessageInputBar.swift` 中的常用功能图标，可在 App 工程中 `*.xcassets` 文件中创建同名图片资源进行替换：
