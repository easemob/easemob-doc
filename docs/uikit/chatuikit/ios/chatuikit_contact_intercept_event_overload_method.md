# 拦截点击跳转事件与 ViewModel 中可重载的方法

## 拦截点击跳转事件

对于好友页面的点击跳转事件，你可以利用原有的逻辑，也可以在原有的逻辑上添加自己的扩展和实现。
...
| `viewContact`   | 查看好友详情页面     | 是     |
| `rightItemsAction`    | 导航右侧按钮点击方法     | 是     |
| `pop`   | 页面返回上一级方法     |   是   |
| `setupTitle`    | 设置不同类型好友页面导航标题     | 是     |
| `receiveContactHeaderAction`    | 好友列表 Header Cell 的点击事件 | 是     |
...
| 方法名 | 用途 | 是否可重载 |
| -------- | -------- | -------- |
| `processFriendDidAgree`    | 收到添加好友对方同意回调。     | 是     |
| `processFriendRequestDidDecline`   | 收到添加好友对方拒绝回调。     | 是     |
...
| `contactEventDidChanged`    | 好友多设备事件变更回调。     | 是     |
