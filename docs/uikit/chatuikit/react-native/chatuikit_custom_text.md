...
UIKit 支持灵活定制界面文本，可修改现有文案或添加完整语言包。

## 概述

UIKit 所有界面文本均存储在 **字符串集（StringSet）** 中，以 **键值对** 形式集中管理。内置中文（`zh-Hans`）和英文（`en`）两种语言，支持以下自定义方式：
