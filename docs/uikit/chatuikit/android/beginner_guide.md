...
<img src="/images/uikit/chatuikit/android/beginner_guide.html">
...
| [创建用户](/document/android/login.html#用户注册) <br/><br/>[实现获取 Token](/document/server-side/easemob_app_token.html)        | - **创建用户**:你可以 [调用 REST API 创建用户](/document/server-side/account_register_open.html)，也可以在 [环信控制台](https://console.easemob.com/user/login) 创建用户。详见 [用户注册文档](login.html#用户注册)。<br/> - **实现获取 Token**：在你的应用服务器集成 [获取 App Token API](/document/server-side/easemob_app_token.html) 和 [获取用户 Token API](/document/server-side/easemob_user_token.html) 实现获取 Token 的业务逻辑，你的应用可以调用自身服务端，从环信服务器获取 Token。   |
| [添加依赖](chatuikit_dependency.html)  | 将单群聊 UIKit [集成到你的应用](chatuikit_dependency.html) ，可从远程或本地添加依赖。   |
| [初始化](chatuikit_initialization.html) <br/><br/> [登录](chatuikit_quickstart.html#第二步-实现代码逻辑)         | - **初始化**：使用单群聊 UIKit 的各项功能前，必须先初始化。传入你应用的 App Key 进行 [初始化](chatuikit_initialization.html)。初始化时，可配置 `ChatOptions` 中的选项，如自动登录。<br/> - **登录**：使用创建的用户登录 IM，包括 [主动登录](/document/android/login.html#主动登录) 和 [自动登录](/document/android/login.html#自动登录)。|
...
| 自定义页面        | 若默认的聊天或会话列表等页面无法满足你的要求，你可以自定义页面，例如，[设置默认的头像或昵称](chatuikit_chat.html#设置头像和昵称)、[设置消息气泡背景](chatuikit_chat.html#自定义消息列表项)、[设置表情功能](chatuikit_chat.html#设置输入菜单相关属性)等。<br/> - [自定义会话列表页面](chatuikit_conversation.html)<br/> - [自定义聊天页面](chatuikit_chat.html)<br/> - [自定义通讯录页面](chatuikit_contactlist.html)<br/> - [自定义联系人详情页面](chatuikit_custom_contact_details.html)<br/> - [自定义群详情页面](chatuikit_custom_group_details.html)。|
