...
历史消息在服务器上的存储时间与你订阅的套餐包有关，详见 [IM 套餐包功能对比](/product/product_package_feature.html)。环信即时通讯 IM 提供消息漫游功能，即将用户的所有会话的历史消息保存在消息服务器，用户在任何一个终端设备上都能获取到历史信息，使用户在多个设备切换使用的情况下也能保持一致的会话场景。默认可获取单聊和群组聊天的历史消息。**若要获取聊天室的历史消息，需联系环信商务**。
