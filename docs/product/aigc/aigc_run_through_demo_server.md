...
登录[环信即时通讯云控制台](/product/enable_and_configure_IM)，点击**添加应用**，填写应用相关信息。
...
在**即时通讯 > 服务概览**页面的**设置**区域，关闭好友关系检查。
...
若使用消息发送前回调功能，你需要在[环信即时通讯云控制台](/product/enable_and_configure_IM)开通该功能，详见[回调配置相关文档](/product/enable_and_configure_IM.html#配置消息回调)。该功能为增值服务，费用详见 [增值服务费用](/product/pricing_policy.html#增值服务费用)。
