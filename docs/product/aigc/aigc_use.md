...
你可以在[环信即时通讯控制台](/product/enable_and_configure_IM)开通 AI 智能功能。

1. 登录[环信即时通讯云控制台](/product/enable_and_configure_IM)，在首页的**应用列表**区域点击目标应用的**操作**一栏中的**管理**。
...
| 用户 ID                            | String      | 是  | 机器人对应的 IM 的用户 ID。目前一个机器人只能绑定一个用户 ID。<br/>**若输入了不存在的用户 ID，配置机器人信息后，会自动创建用户。**<br/>用户 ID 长度不可超过 64 个字节。不可设置为空。支持以下字符集：<br/>- 26 个小写英文字母 a-z；<br/>- 10 个数字 0-9；<br/>- “\_”, “-”, “.”。 <br/><Container type="notice" title="注意"><br/>- 请勿使用大写英文字母 A-Z；<br/>- 请确保同一个 app 下，用户 ID 唯一；<br/>- 用户 ID 为公开信息，请勿使用 UUID、邮箱地址、手机号等敏感信息。</Container>   | 
...
你可以在[环信即时通讯控制台](/product/enable_and_configure_IM)的左侧导航栏选择 **即时通讯** > **AI 智能** > **数据统计**，查看机器人账号消耗的 token 数和消耗趋势。
