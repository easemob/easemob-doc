...
移动端 SDK 默认连接国内数据中心。目前采用随机策略，将用户分配到国内 1 区或国内 2 区，这两个集群的服务能力没有区别。

Web 端和桌面端 SDK 集成时，需要根据应用所在的数据中心填写对应的 WebSocket 地址。
...
移动端 SDK 在配置海外集群的 App Key 后，会自动连接到对应的海外集群。
...
### 2. 如何确认我的应用使用的是哪个数据中心？

你可以登录环信控制台，在 **应用概览** 页面查看应用所在数据中心及对应服务器域名。

### 3. 国内 1 区和国内 2 区有什么区别？

当前国内 1 区和国内 2 区在服务能力上没有区别。移动端 SDK 默认采用随机策略接入其中一个集群。
...
### 6. 海外加速节点默认带宽是多少？是否可以调整？

海外加速节点默认带宽为 **1 Mbps**。如果业务流量更高，可以根据实际需求配置更高带宽，具体请联系环信商务。

### 7. Web 端、桌面端和小程序是否需要根据数据中心配置不同地址？
