...
对于发送后回调，每条回调请求发送失败仅重试一次，重试失败后即丢弃。若有特殊需求不能丢失回调消息的情况下，请联系环信商务开通回调异常缓存功能。
...
| 告警策略 | 是 | <br/> - **告警间隔**：触发告警的时间间隔，单位为 5 分钟。 <br/> - **触发次数**：告警间隔内触发告警的回调失败次数。   |
