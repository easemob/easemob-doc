...
| 群组类型             | String   | 否       | <br/> - （默认）**公开群**：公开群可以被搜索到，用户可以申请加入公开群。- **私有群**：私有群无法被搜索到，用户只有在群成员邀请后才能加入。 |
...
开通消息人工审核后，你可以对群组的消息、属性和成员进行实时审核管理操作。
...
- 将群成员加入黑名单；
- 将群成员加入禁言列表；
- 作为 app 管理员发送和撤回消息以及发送离线通知。// TODO：向测试要一些数据，然后重新截图
