...
环信即时通讯 IM 支持消息管理、用户管理功能、离线推送等功能。
...
使用多端登录前，你需要在环信即时通讯控制台的 **即时通讯** > **功能配置** > **功能配置总览** > **基础功能** 页面上查找 **多端多设备在线**，开启该功能。此外，可以在 **基础功能**页签下点击 **多端多设备在线** 操作栏中的 **设置**，在弹出的对话框中设置各端设备的数量。
...
| 本地存储       | 接收的消息存储在本地。         |
| 历史消息       | 环信服务器可存储消息。 |
| 离线消息推送       | <br> - 用户离线时，可向消息接收方发送推送通知。<br> - 支持离线消息存储，默认为 14 天。 |
...
| 群组/聊天室总数上限  | 100,000个  | 无限制     |
| 成员数上限          | 3000人/群 | 10,000 人/聊天室   |
...
| 历史消息存储         | 支持     |  支持    |
| 查看漫游消息         | 默认开通     | 联系商务开通     |
| 是否支持查看加入前漫游消息          |  支持（联系商务开通）    | 支持（联系商务开通）    |
| 消息未读计数          | 支持     | 不支持   |
