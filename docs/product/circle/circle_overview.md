...
消息话题（Thread）是一个基于频道中某一条消息展开的讨论，用户可以某条消息为中心，与其他对此条消息感兴趣的朋友一起聚焦沟通。
...
- 每条消息只能创建一条消息话题。每个频道支持的消息话题数量无限制。
...
|普通成员|社区中普通成员|社区的普通成员可以在频道和消息话题中收发和查看消息。|
...
|功能|描述|
| :- | :- |
|发送消息|用户可以给指定用户、频道或消息话题发消息/图片/文件。|
|撤回消息|用户可以撤回自己发送的消息。|
