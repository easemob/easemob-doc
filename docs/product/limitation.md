...
- **消息附件/文件**：消息附件，例如图片、音频、视频和其他文件默认存储 7 天。若要上调该限制，请联系商务。消息附件的大小及存储时间限制与群组共享文件的相同。如果消息附件的其中一个限制进行了上调，群组共享文件的对应限制也会随之自动调整，反之亦然。
...
- **表情计数原则**：同一消息表情（Reaction）被多位用户添加时，Reaction 数量计为 1。
