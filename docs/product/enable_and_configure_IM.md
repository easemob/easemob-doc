...
![img](/images/privitization/deploy_service.jpg)

## 设置消息撤回（REST & 客户端）

1. 在环信即时通讯云的左侧导航栏中，选择**即时通讯** > **功能配置** > **功能配置概览**。在 **功能配置概览** 页面下，搜索找到消息撤回（REST&客户端），单击右侧的 **设置**（已开通状态下）。
2. 在弹出的 **消息撤回时间设置** 对话框中，设置消息可撤回时长。端消息撤回时长和 REST 消息撤回时长均不能超过 7 天。
3. 单击 **确定** 保存配置。

![img](/images/privitization/deploy_rest.jpg)
...
2. 在左侧导航栏，选择 **即时通讯** > **功能配置** > **消息推送** > **证书管理**，进入 配置证书页面。

![img](/images/privitization/deploy_push_settings.jpg)

3. 点击 **添加推送证书** 按钮，打开 **添加推送证书** 对话框。


![img](/images/privitization/deploy_push-settings_window.jpg)

4. 在 **添加推送证书** 对话框中，填写各厂商平台的相关推送信息，点击 **保存** 按钮，完成证书配置。

![img](/images/privitization/deploy_push-settings_form.jpg)
...
2. 在左侧导航栏，选择 **即时通讯** > **功能配置** > **消息回调**，进入回调服务页面。

![img](/images/privitization/deploy_push_callback.jpg)

3. 点击 **添加回调规则** 按钮，打开回调配置对话框，在回调配置对话框中，填写回调相关配置信息，点击 **保存** 按钮，完成回调配置，具体配置内容说明见 [回调配置](/document/server-side/callback_overview.html#实现步骤)。

![img](/images/privitization/deploy_push_window.jpg)
...
![img](/images/privitization/deploy_sensitive_words.jpg)
...
2. 在 **安全配置** 页面，单击 **添加**，在弹出的 **添加 IP** 对话框中，输入 IP 地址。
:::notice
...
:::
3. 单击 **保存** 完成配置。
![img](/images/privitization/deploy_ip_whitelist.jpg)
...
![img](/images/privitization/deploy_ip_allow_list.jpg)
...
![img](/images/privitization/deploy_user_manage.jpg)
