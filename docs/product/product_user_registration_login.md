...
Before logging in to the SDK, you must first create an EasyIM user. Before creating the user, set the user registration mode on the **EasyIM > Basic Features** > **Users** page of the [EasyIM Console](https://console.easemob.com/user/login).
...
2. **Create users in the EasyIM Console**
   ...
   You can create users in production or test environments in the [EasyIM Console](https://console.easemob.com/user/login). For details, see [Create a user](/product/console/operation_user.html#create-a-user).
...
2. When you obtain a token, the value passed for `ttl` determines the token validity period. If this parameter is not passed, the token validity period configured on the **User Management** page of the [EasyIM Console](https://console.easemob.com/user/login) is used. The default is 60 days. If the value is set to `0`, the token never expires.
:::
