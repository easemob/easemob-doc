...
| `templateId`             | String | 小米推送私信模板 ID，使用私信模板时必填，且须同时提供对应的 `channelId`。新版私信模板的标题包含变量时，必须保留不少于 2 个非空固定字符，例如 `应用名称·{$title$}`。详见 [小米推送模板接入指南](https://dev.mi.com/xiaomihyperos/documentation/detail?pId=2314)。 |
