...
调用前，请参考 [支持格式与参数](#支持格式与参数)，确认当前语音消息的格式是否在支持范围内。
...
| `audioParams` | `AudioParams` | 否 | 语音参数。`PCM` 文件必须传入；`MP3`、`AMR`、`WAV`、`M4A` 和 `AAC` 文件通常可传 `null`。 |
...
`WAV`、`M4A` 和 `AAC` 格式由服务端直接识别和处理，小程序 SDK 仅负责将文件数据透传至服务端，不进行本地解析或参数校验。因此，在调用 `voiceFileToText` 时，对于这些格式的文件，`audioParams` 参数传 `null` 即可。详见 [支持格式与参数](#支持格式与参数)。
