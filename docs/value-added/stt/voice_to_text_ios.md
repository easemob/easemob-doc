...
语音参数 `voiceParam` 仅用于本地语音文件转文字。对于本地 `PCM` 文件，由于文件本身不包含完整音频头信息，必须传入 `voiceParam`；其他格式通常可传 `nil`。具体配置方式请参考 [将本地语音文件转换为文本](#将本地语音文件转换为文本)。
...
转换成功后，也可通过 `EMVoiceMessageBody.text` 获取对应文本。为获得更优性能，**推荐优先使用标准的 `MP3` 格式语音消息**。
...
`EMVoiceParam` 用于描述语音文件的格式、采样率、采样位深和声道数。对于无文件头的 `PCM` 文件，必须传入该参数；对于 `MP3`、`AMR`、`WAV`、`M4A` 和 `AAC` 等格式信息完整的文件，通常可传 `null`。
...
- `PCM` 音频不支持通过 [语音消息转文字接口](#将语音消息转换为文本) 直接转换，需要调用 [本地文件转文字接口](#将本地语音文件转换为文本)，并且传入 `EMVoiceParam` 参数。
...
`WAV`、`M4A` 和 `AAC` 格式由服务端直接识别和处理，iOS SDK 仅负责将文件数据透传至服务端，不进行本地解析或参数校验。因此，在调用 `voiceFileToText:voiceParam:completion:` 时，对于这些格式的文件，`voiceParam` 参数传 `nil` 即可。