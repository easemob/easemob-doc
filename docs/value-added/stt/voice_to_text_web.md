...
- 已完成 [SDK 初始化](/document/web/initialization.html)，并成功 [登录](/document/web/login.html)。
- 已联系商务开通语音转文字服务。
- 已具备 [发送](/document/web/message_send.html#发送语音消息) 和 [接收语音消息](/document/web/message_receive.html#接收语音消息) 的基础集成能力。
...
`AudioParams` 用于描述语音文件的格式、采样率、采样位深和声道数。对于无文件头的 `PCM` 文件，必须传入该参数；对于 `MP3`、`AMR`、`WAV`、`M4A` 和 `AAC` 等格式信息完整的文件，通常可传 `null`。
...
- `PCM` 音频不支持通过 [语音消息转文字接口](#将语音消息转换为文本) 直接转换，需要调用 [本地文件转文字接口](#将本地语音文件转换为文本)，并且传入 `AudioParams` 参数。
