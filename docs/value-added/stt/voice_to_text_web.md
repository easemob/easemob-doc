...
本文介绍如何使用 SDK 将语音消息或本地语音文件转换为文字。
...
- 本地语音文件转文字：将浏览器中的 `File` 对象转换为文本。
- 语音参数配置：为无文件头的本地语音文件补充格式、采样率、采样位深和声道数等参数。
- 读取转文字结果：转换成功后，可通过接口返回值中的 `res.data.text` 获取识别文本。

接入前，建议先阅读 [支持格式与参数](#支持格式与参数)，确认接口支持的音频格式范围以及 `AudioParams` 的传参要求。
...
- 本地语音文件转文字：业务层传入浏览器 `File` 对象，SDK 上传文件后调用 [接口](#将本地语音文件转换为文本) 完成转换。

时序图如下所示：

![img](/images/voicetotext/voice_to_text_sequence_web.png)
...
3. IM SDK 使用 `FormData` 和 `XMLHttpRequest` 上传 `File`。
...
[语音消息转文字](#将语音消息转换为文本) 与 [本地语音文件转文字](#将本地语音文件转换为文本) 支持的音频格式不完全相同。对于缺少文件头信息的 `PCM` 文件，必须同时传入 `audioParams`，以便 SDK 正确解析原始音频数据。
...
`AudioParams` 用于描述语音文件的基础属性，包括格式、采样率、位深和声道数，有助于准确地解析语音内容。调用本地语音文件转文字接口时，对于格式信息完整的文件，通常可不传，但对于缺少文件头信息的音频文件，需要通过该参数补充语音信息。
...
调用前，请参考 [支持格式与参数](#支持格式与参数)，确认当前语音消息的格式是否在支持范围内。转换成功后，可通过返回值中的 `res.data.text` 获取识别结果。
...
该接口支持的文件格式、`audioParams` 是否必传，以及 `PCM` 的特殊要求，可统一参考 [支持格式与参数](#支持格式与参数)。此外，要求待转换文件的大小不超过 10 MB，且时长不超过 60 秒。
