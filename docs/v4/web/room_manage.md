...
- **普通加入**：仅需加入指定聊天室时，调用 `joinChatRoom` 方法并传入聊天室 ID。
- **携带扩展信息加入**：需要在加入聊天室时传递自定义业务信息，或控制是否退出已加入的其他聊天室时，在调用 `joinChatRoom` 方法时设置相应参数。
...
调用 `joinChatRoom` 方法时，可以设置加入聊天室时携带的自定义扩展信息，并控制用户加入当前聊天室时，是否退出已加入的其他聊天室。
...
用户成功加入聊天室后，聊天室内的其他成员会收到 `onChatRoomEvent#memberPresence` 事件。如果加入聊天室时传入了扩展信息，其他成员可以通过该事件获取。
