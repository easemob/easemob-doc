...
:::tip
发送聊天室消息时，是否创建对应的本地聊天室会话由 `EMOptions#enableChatroomConversation` 控制。该配置默认为 `NO`，即默认不创建本地聊天室会话，但不影响消息发送。如需创建，请在 SDK 初始化前将该配置设为 `YES`。详见 [初始化时设置会话相关选项](conversation_list.html#初始化时设置会话相关选项)。
:::
