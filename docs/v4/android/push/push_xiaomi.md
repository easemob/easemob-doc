...
| 小米私信模板 ID  | String | 否     | 小米推送私信模板 ID，使用私信模板时必填，且须同时提供对应的 `Channel ID`。新版私信模板的标题包含变量时，必须保留不少于 2 个非空固定字符，例如 `应用名称·{$title$}`。详见 [小米推送模板接入指南](https://dev.mi.com/xiaomihyperos/documentation/detail?pId=2314)。<br/>**配置示例**：下图展示了在 [小米开放平台](https://dev.mi.com/platform) 使用私信模板配置推送标题和内容的示例。|
| Action      | String | 否     | 点击通知后的动作。该参数仅对离线推送有效。           |

下图展示了在 [小米开放平台](https://dev.mi.com/platform) 使用小米推送“私信模板”配置推送标题和内容的示例。标题示例 `应用名称·{$title$}` 包含变量，并保留了不少于 2 个非空固定字符。

![img](/images/android/push/template_private_xiaomi.png)
