...
:::tip
接收聊天室消息时，是否创建对应的本地聊天室会话由 `EMOptions#setEnableChatroomConversation` 控制。该配置默认为 `false`，即默认不创建本地聊天室会话，但不影响消息接收。如需创建，请在 SDK 初始化前将该配置设为 `true`。详见 [初始化时设置会话相关选项](conversation_list.html#初始化时设置会话相关选项)。
:::
