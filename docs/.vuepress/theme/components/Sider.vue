...
<template>
    <div class="sider-container">
        ...
        <div class="sider-menu">
            <div class="item" v-for="group in sidebar"
                v-if="(group.show === undefined || group.show !== false) && (!language || ((!group.only && !group.except) || (group.only && group.only.indexOf(language) !== -1) || (group.except && group.except.indexOf(language) === -1)))">
                <h2>{{ group.title }}</h2>
                <ul>
                    <li v-for="item in group.children" :class="{ active: item.link == file }"
                        v-if="(item.show === undefined || item.show !== false) && (!language || ((!item.only && !item.except) || (item.only && item.only.indexOf(language) !== -1) || (item.except && item.except.indexOf(language) === -1)))">
                        <template v-if="item.children">
                            <SiderChild :key="item.children[0].link + 'sider-child'" :menu="item" />
                        </template>
                        <template v-else>
                            <router-link :to="item.link.slice(0, 1) == '/' ? item.link : (path + item.link)"
                                v-if="(item.show === undefined || item.show !== false) && (!language || ((!item.only && !item.except) || (item.only && item.only.indexOf(language) !== -1) || (item.except && item.except.indexOf(language) === -1)))">{{
        item.text
                                }}</router-link>
                        </template>
