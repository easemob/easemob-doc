...
| 214    |   USER_LOGIN_TOO_MANY_DEVICES   | 用户登录设备数超过限制。该错误出现的场景如下：用户在多端多设备场景下登录时，若已达到登录设备数量上限，用户之前登录的设备自动登录时会提示该错误。 |
...
| 220    |       USER_DEVICE_CHANGED       | 用户登录设备与上次不一致。该错误出现的场景如下：单设备登录情况下，用户上次登录设备 A，实现自动登录。用户本次若手动登录设备 B，则能成功登录，踢掉设备 A 上的登录。若用户开启了不踢掉另外设备上的登录时，用户手动登录设备 B 能成功登录，但再次自动登录设备 A 时登录失败且提示该错误。因此，该错误在打开不踢掉另外设备上的登录开关后才会出现。 |
