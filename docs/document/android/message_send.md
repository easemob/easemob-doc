...
- 大图：SDK 在非原图发送场景下基于原图压缩后生成的图片资源。压缩规则为短边不超过 720 像素，压缩质量为 85%，通常用于聊天详情页展示。
- 缩略图：服务端基于原图进行等比压缩后的图片。压缩规则为短边默认为 170 像素。缩略图的压缩方式和尺寸可在[控制台进行配置](product/basic_message.html#图片消息缩略图)。此类图片通常用于会话列表、聊天列表等轻量展示场景。
