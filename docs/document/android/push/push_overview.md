...
要体验离线推送功能，请点击[这里](https://github.com/easemob/easemob-demo-android)下载即时通讯 IM 的 demo。
...
除了满足用户离线条件外，要使用第三方离线推送，你还需在**环信即时通讯云控制台**配置推送证书信息，例如，对于华为推送，需配置**证书名称**和**推送密钥**，并调用客户端 SDK 提供的 API 向环信服务器上传 device token。
...
**推送证书**：推送证书是环信服务器用于判断目标设备使用哪种推送通道的唯一条件。发送推送通知前需要在**环信即时通讯云控制台** 配置 App 的推送证书，需填写证书名称（或者 App Key），确保与 Android 终端设备上传的证书名称一致。该步骤必须在登录环信 IM SDK 成功后进行。
...
关于如何获取推送 Token 并上传至环信服务器，详见各推送服务的集成文档，例如 对于 FCM 推送，请参阅 [FCM 推送集成文档](push_fcm.html#步骤四-fcm-推送集成)。

## 推送模板

推送模板是离线推送的高级功能。使用前，你需要在**环信即时通讯控制台 > 即时通讯 > 功能配置 > 功能配置总览**页面激活。高级功能包括[推送通知方式](push_notification_mode_dnd.html#推送通知方式)、[免打扰模式](push_notification_mode_dnd.html#免打扰模式)和[推送模板](push_display.html#使用推送模板)。**如需关闭推送高级功能必须联系商务，因为该操作会删除所有相关配置。**


## 多设备离线推送策略

多设备登录时，可在**环信即时通讯控制台 的 证书管理** 页面配置推送策略，该策略配置对所有推送通道生效：
...
3. 若使用[推送高级功能](#离线推送高级功能)，需在 **环信即时通讯控制台** 上激活。
4. 各推送使用的条件：
...
5. 使用消息推送前，需要你在对应的厂商推送服务上注册项目，并将设备的推送证书上传到 **环信即时通讯云控制台**。
