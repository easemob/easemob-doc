...
**如需关闭推送模板必须联系商务，因为该操作会删除所有相关配置。**

你可以通过以下两种方式设置：

- [调用 REST API 配置](/document/server-side/push.html#使用推送模板)。
- 在[环信控制台](https://console.easemob.com/user/login)设置推送模板，详见[控制台文档](/product/console/push_template.html)。
...
:::tip
1. 设置推送模板为推送的高级功能，使用前需要在[环信控制台](https://console.easemob.com/user/login)的 **功能配置 > 基础功能 > 消息** 页面开通。如需关闭该功能联系商务，因为该操作会删除所有相关配置。
...
内置参数的介绍，详见[环信制台文档](/product/enable_and_configure_IM.html#使用默认推送模板)。
