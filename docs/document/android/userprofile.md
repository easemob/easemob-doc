...
A：你可以调用 [客户端](#设置当前用户的所有属性) 或 [RESTful API](/document/server-side/user_attribute_set.html) 设置用户昵称，例如 Android 为 `updateOwnInfo`，然后通过 [客户端](#获取用户的指定属性) 或 [RESTful API](/document/server-side/user_attribute_obtain_single.html) 获取用户属性，例如 Android 为 `fetchUserInfoByAttribute`。
