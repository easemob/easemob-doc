...
**群聊会话只支持消息已读回执，不支持送达回执**。群成员在发送消息时，可以设置该消息是否需要已读回执。要使用该功能，你需要[在环信即时通讯云控制台上开通该功能](enable_and_configure.html#设置群消息已读回执)，具体费用详见[产品价格](/product/pricing.html#增值服务费用)。
...
使用环信即时通讯 IM Android SDK 可以实现消息的送达回执与已读回执。核心方法如下：

- `EMOptions#setRequireDeliveryAck` 开启消息送达回执；
- `EMOptions#setRequireAck` 开启消息已读回执功能；
- `EMChatManager#ackMessageRead` 发出指定消息的已读回执；
- `EMChatManager#ackGroupMessageRead` 发出群组消息的已读回执。
...
单聊既支持单条消息已读回执，也支持[会话已读回执](conversation_read_receipt.html)。我们建议你结合使用这两种回执：
...
| 字段       | 描述   | 
| :--------- | :----- | 
| `EMMessage#isUnread` | 用户是否已读了该消息。如果是自己发送的消息，该字段的值固定为 `false`。| 
