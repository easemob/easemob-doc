...
一次可订阅多个用户的在线状态。批量订阅用户在线状态分为同步订阅和异步订阅：
- 同步订阅：服务器会首先订阅用户在线状态，然后返回该 API 的响应，这样可以保证实时获取到订阅用户在线状态列表。
  如果订阅用户的数量小于 5 人，默认为同步订阅。
  
- 异步订阅：服务器会首先将订阅用户在线状态的任务放到队列中，然后返回该 API 的响应，不能保证能实时获取到订阅用户在线状态列表。
  如果订阅的用户数量大于 5 人，默认为异步订阅。如需要同步订阅，可以联系环信商务进行开通。
