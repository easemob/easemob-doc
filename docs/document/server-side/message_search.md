...
要使用服务端消息搜索功能，需要在环信控制台开通，详见 [开通说明](/product/console/purchase_value_added.html#消息搜索)。开通后，系统会为你的应用创建搜索索引资源并开始同步消息数据。
...
| `searchExt` | String | 否 | 控制是否对消息扩展字段 `ext` 执行搜索：<br/> - （默认）`none`：不搜索 `ext`。<br/> - `with`：同时搜索消息内容和 `ext`。<br/> - `only`：仅搜索 `ext`。<br/><br/>关于 **`ext` 搜索** 的说明*：<br/>1. **开通功能**：`ext` 搜索默认不支持，需联系环信商务开通。<br/>2. **配置白名单**：开通后，需联系环信商务指定可搜索的 `ext` 字段。仅白名单内的字段会参与搜索（默认白名单为空）。<br/>3. **确认生效范围**：白名单配置仅对配置生效后新写入或更新的消息生效。历史消息如需搜索，需联系商务同步。|
...
**高亮片段截取规则**：高亮片段以命中关键词所在位置为中心，从匹配文本中截取。命中结果较多时，系统按相关性返回最相关的片段，实际返回数量不超过 `highlightNumberOfFragments`；当 `highlightNumberOfFragments` 取值为 `0` 时，返回完整字段内容而非片段。
