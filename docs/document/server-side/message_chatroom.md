...
- **发送频率**
  ...
  对于单个 app，该 RESTful API 存在以下三个限制：
    - 10 个聊天室/次 
    - 100 条/秒：每秒最多可向聊天室发送 100 条消息。例如，你每次向 10 个聊天室发送消息，即发送了 10 条消息，你每秒最多可调用 10 次该接口。第 11 次调用时，则报 403 错误。
    - 20 次/秒：每秒最多可调用 20 次该 API。例如，你每次调用该 API 向单个或多个聊天室发送消息，可调用 20 次，第 21 次调用时会报 429 错误。
