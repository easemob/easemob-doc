...
| 429         | resource_limited    | You have exceeded the limit of the community edition,Please upgrade to the enterprise edition | 请求 body 中 `grant_type` 为 `inherit`，`autoCreateUser` 为 `true`（用户不存在时，自动注册用户） ，在使用注册用户的数量超过版本的限制 | [在环信控制台提升注册用户数量上限](/product/console/basic_user.html#注册用户总数上限)。 |
