...
- 若 app 需从其他即时通信系统迁移到环信即时通讯 IM 时，可调用接口导入单聊和群聊的历史消息到环信服务器。
- 单聊和群聊只支持每次导入一条消息。   
- 导入消息后会生成对应的会话，你可以设置会话是否为已读状态（`is_ack_read` 参数）
- 对于附件类消息，例如，图片、音频、视频和文件消息，你可以设置是否需要下载附件并上传到服务器。
- 拉漫游时可拉到导入的消息。
- 导入消息不触发 [消息回调](callback_overview.html)。
