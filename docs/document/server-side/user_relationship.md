...
- 从用户的好友列表中移除一个用户，解除这两个用户的好友关系。例如，用户 A 将 用户 B 移除好友后，双方均会从彼此的好友列表消失。
- 调用该接口会触发移除好友的发送后回调事件，详见[回调事件文档](callback_contact.html#删除好友)。
...
| `friend_username` | String | 是       | 被移除的好友的用户 ID。 |
...
- 若当前用户为好友设置了好友备注，调用该接口后，好有备注会从服务端删除。// TODO：是否删除客户端的好友备注，目前不明确。
...
| 参数              | 类型   | 是否必需 | 描述           |
| :---------------- | :----- | :------- |:-------------|
| `remark`  | String | 是   | 好友备注。好友备注的长度不能超过 100 个字符。不传这个参数会将好友备注清空。 |
...
| 参数        | 类型  | 是否必需 | 描述                                                         |
| :---------- | :---- | :------- | :----------------------------------------------------------- |
| `usernames` | Array | 是       | 要加入黑名单的用户 ID，例如 ["user1", "user2"]，建议每次不超过 100 个用户。|
...
|  - `relation`      | String     | 对象类型，用户是否在黑名单中：<br/> - `blacklist`：是；<br/> - `not_blacklist`：否。| // TODO
