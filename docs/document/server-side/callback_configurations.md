...
仅单聊和群组聊天支持表情回复 Reaction。如要获取回调事件，在[控制台](https://console.agora.io/)上的**发送后回调**对话框（**即时通讯** > **功能配置** > **消息回调**>**添加回调地址**）中无需单独配置**回调类型**，你只需选择单聊或群组聊天对应的消息类型即可。例如，如果需要单聊文本消息的 Reaction，你需要在**回调类型**下拉列表框选中 **单聊消息 > 文本消息**，服务器发送单聊文本消息相关回调事件时会发送该消息相关的 Reaction 事件。
...
仅群组聊天支持子区 Thread。如要获取回调事件，在[控制台](https://console.agora.io/)上的**发送后回调**对话框（**即时通讯** > **功能配置** > **消息回调**>**添加回调地址**）中无需单独配置**回调类型**，你只需选择群组对应的消息类型即可。例如，如果需要群组文本消息的 Thread，你需要在**回调类型**下拉列表框选中 **群组消息 > 文本消息**，服务器发送群组文本消息相关回调事件时会发送该消息的相关 Thread 事件。
