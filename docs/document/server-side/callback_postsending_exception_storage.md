...
开通该服务后，当发送后回调失败且满足异常缓存条件时，系统会按时间片对失败的回调消息进行归档存储，**并按每 10 分钟生成一个 date key，用于标识一个异常回调集合**。你可以先调用 [查询接口](#查询异常缓存数据) 获取可补发的数据集合，再根据返回结果调用 [补发接口](#补发异常回调数据) 进行重投，以完成异常消息的补偿处理。
...
如需使用回调异常缓存，请先在控制台开通对应服务，详见 [环信控制台开通入口](/product/console/basic_webhook.html#回调异常缓存)。
...
当 HTTP 状态码非 `200` 时，表示请求失败。你可以参考本文档中的 [响应状态码](#响应状态码) 排查原因。
...
当 HTTP 状态码非 `200` 时，表示补发请求失败。你可以参考 [响应状态码](#响应状态码) 了解可能原因。
