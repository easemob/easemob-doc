...
即时通讯 IM 支持发送 app 全局广播消息和聊天室全局广播消息。
...
**该功能默认关闭，如果需要，请联系环信商务开通发送广播消息功能。**开通后，你可以发送 app 全局广播消息和聊天室全局广播消息。

- 广播消息只向 app 下的在线用户发送。
- 广播消息不支持离线存储，即离线用户收不到这些消息。
- 广播消息写入服务端会话列表，默认不支持漫游功能。**如果需要，请联系商务开通。**

**发送频率**：每 30 分钟限 1 次，每天限 3 次（可联系商务调整该上限），每秒最多向 1000 个目标用户下发消息。
...
**发送频率**：每分钟限发 10 次，而且每天限发 100 次广播消息。
