...
- 3 个群/次
- 20 条/秒/App Key： 每秒最多可向单个 app 中的群组发送 20 条消息。例如，你每次向 3 个群组发送消息，即发送了 3 条消息，你每秒最多可调用 7 次。第 8 次调用时，则报 403 错误。  
- 20 次/秒：每秒最多可调用 20 次该 API。例如，你每次调用该 API 向单个或多个群组发送消息，可调用 20 次。第 21 次调用时会报 429 错误。 
