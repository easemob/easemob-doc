...
- 使用前需先在[环信控制台](https://console.easemob.com/user/login)打开相应应用的开放注册开关，即在控制台的 **应用管理** 页面点击目标应用的 **操作** 一栏中的 **管理**，然后选择 **功能配置 > 基础功能** > **用户**，将 **用户注册模式** 设置为 **开放注册**。
- 调用该 API 无需传入 token。
