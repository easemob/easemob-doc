...
为方便用户在聊天过程中对文字消息进行翻译，环信即时通讯 im_flutter_sdk 集成了 Microsoft Azure Translation API，支持在发送或接收消息时对 **文本消息** 进行按需翻译或自动翻译：
...
文本翻译为增值服务，如需使用请先在 [环信控制台开通](/product/console/purchase_value_added.html#消息翻译)。服务费用为每月 500 元，含 200 万字符额度。

单次翻译请求最多支持 10,000 字符，按 **源文本字符数 × 目标语言数量** 计费。例如，将 500 字符翻译为 4 种语言，计费字符数为 500 × 4 = 2,000 字符。
