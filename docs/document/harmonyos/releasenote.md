...
- 新增 [获取群成员信息列表](group_manage.html#获取群成员列表) 接口。
  
### 优化

- 修改 Token 即将过期事件 [onTokenWillExpire](connection.html#监听连接状态) 的触发时机。SDK 会在 Token 有效期达到 80% 左右时（之前版本为 50% ）回调即将过期通知。
