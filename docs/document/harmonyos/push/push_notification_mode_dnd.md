...
在本地调用 `PushManager#setSilentModeForConversation` 设置会话的推送通知方式，该操作会通过多设备事件 `MultiDevicesListener#onConversationEvent` 回调。此时，参数 `event` 的值为 `CONVERSATION_MUTE_INFO_CHANGED`，该事件定义在 `MultiDevicesEvent` 中。
