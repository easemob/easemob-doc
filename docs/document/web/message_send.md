...
- 对于图片消息，如果要使用缩略图功能，需要提供oss环境，环信服务器会自动生成图片缩略图；对于视频消息，视频首帧为缩略图。
...
对于图片消息，服务器会根据用户设置的 `thumbnailHeight` 和 `thumbnailWidth` 参数自动生成图片的缩略图。若这两个参数未传，则图片的高度和宽度均默认为 170 像素。你也可以在 **环信即时通讯控制台** 的 `应用概览>应用详情` 页面的 `应用设置` 区域修改该默认值。
