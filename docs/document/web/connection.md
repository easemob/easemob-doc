...
登录成功后，SDK 会维护长连接，并仅在网络恢复、心跳失败、连接异常关闭等可恢复场景下尝试自动重连。
...
并非所有连接断开场景都适合依赖自动重连。对于 Token 过期、鉴权失败、账号状态变化、设备限制或服务配额限制等场景，业务层需要根据连接事件中的错误信息进行处理。自 SDK 5.1.2 起，用户被踢下线等正常退出后，Demo 页面会显示对应状态；页面恢复或网络恢复时，SDK 不再触发自动重连，如需继续使用请重新登录。
...
| 当前运行环境离线 | 无固定错误码；原因通常为 `offline` | 不需要立即重新登录 | 对于仍处于可恢复连接周期的会话，SDK 会暂停重连，并在网络恢复后继续尝试；已正常退出的会话不会恢复连接。 |
...
- 网络断开后，SDK 会暂停重连；仅对仍处于可恢复连接周期的会话，网络恢复后才会继续尝试恢复连接。自 SDK 5.1.2 起，用户被踢下线等正常退出后，页面恢复或网络恢复不会触发自动重连，需重新登录。
