...
2. 发送方调用 `sendMessage` 方法发送该消息。SDK 会将图片上传至环信服务器，如果私有部署提供oss，服务器自动生成图片缩略图。
...
3. 发送方调用 `sendMessage` 方法发送消息。SDK 会将视频文件上传至消息服务器。若需要视频缩略图，私有部署需要提供oss环境，且需自行获取视频首帧的路径，将该路径传入 `createVideoMessage` 方法。
