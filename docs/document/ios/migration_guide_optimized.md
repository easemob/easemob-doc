# 环信 iOS IM SDK 4.x 到 5.0 迁移指南

## 升级总览

iOS IM SDK 5.0 是一次源代码不兼容的大版本升级，主要涉及以下四个方面：
...
:::tip
**升级要求：** iOS IM SDK 5.0 包含不兼容的 Objective-C API 变更。更新 SDK 后必须重新编译，并重点验证 Token 登录、数据同步、会话未读数、单聊和群聊已读回执、群组创建和配置更新、联系人、附件消息及推送设置。
:::
...
iOS SDK 5.0 不再提供自动登录配置、自动登录状态和自动登录完成回调。应用冷启动后，需要自行管理用户 ID 与 Token，并在适当时机主动调用 `loginWithUsername:token:completion:`。
...
| `EMClientDelegate#userAccountDidLoginFromOtherDevice:`、无参数的 `userAccountDidLoginFromOtherDevice` | `userAccountDidLoginFromOtherDeviceWithInfo:` | 通知当前账号在其他设备登录。5.0 统一通过 `EMLoginExtensionInfo` 返回新登录设备及扩展信息。 |

`isLoggedIn` 表示当前是否已经登录，`isConnected` 表示是否连接至聊天服务器。二者都不能替代应用对用户凭证和 Token 生命周期的管理。

### 密码登录下线

// TODO：下面的第二句是否要改为“密码校验和 Token 获取等账号管理操作需要由业务服务器完成。” 因为用户注册无需由业务服务器完成，调用 REST API 就可以。

iOS SDK 5.0 仅保留 Token 登录方式。用户注册、密码校验和 Token 获取等账号管理操作需要由业务服务器完成。
...
| 同步 `loginWithUsername:token:` | `loginWithUsername:token:completion:` | 使用 Token 同步登录并直接返回 `EMError`；5.0 仅保留异步 completion 版本。 |
...
`renewToken:completion:` 仍用于异步更新当前登录会话的 Token；同步 `renewToken:` 也仍在 5.0 公共头文件中保留，但业务代码应优先使用 completion 版本处理错误。

### 登录与数据库打开解耦

iOS SDK 5.0 新增本地数据库打开回调。数据库打开后即可读取当前账号的本地数据，不必等待会话、联系人或已加入群组的服务端同步完成，有助于加快冷启动时的首屏展示。
...
iOS SDK 5.0 新增登录后自动数据同步机制。应用应在初始化 SDK 前通过 `EMOptions#dataSyncType` 指定同步的数据类型，并通过 `EMClientDelegate` 监听同步进度。同步完成后，从各模块的本地接口读取数据。
...
| `EMOptions` | `dataSyncType` | 设置登录后自动同步的数据类型；必须在 `initializeSDKWithOptions:` 前配置。当前 5.0 代码实现的默认值为 `EMDataSyncTypeConversations`。 |
...
// TODO：需要删掉下面的 tip 中的 "需要注意的是，当前公共头文件注释仍将默认值写为 `EMDataSyncTypeNone`，与实现不一致。" 描述吗？

:::tip
当前 5.0 代码实现中，`EMOptions#dataSyncType` 默认为 `EMDataSyncTypeConversations`，因此配置时仅自动同步会话，不会自动同步联系人或已加入群组。需要注意的是，当前公共头文件注释仍将默认值写为 `EMDataSyncTypeNone`，与实现不一致。为避免 SDK 版本或注释差异影响业务，建议应用在初始化前始终显式设置 `dataSyncType`。
:::
...
| 直接设置 `EMChatMessage.isRead` | 无公开 setter；使用未读数清理接口 | `isRead` 在 5.0 中变为只读属性，由 SDK 内部维护。 |
...
iOS SDK 5.0 将单聊和群聊的实时已读回执统一通过 `EMChatManagerDelegate` 回调，不再分别使用单聊和群聊回调。
...
SDK 5.0 新增 `EMMessageReadReceipt`，用于统一描述消息已读回执：
...
| 4.x API | 5.0 API | 说明 |
| :--- | :--- | :--- |
| `EMChatMessage.isReadAcked` | `EMChatMessage.isPeerRead` | 判断消息对端是否已读；5.0 为只读属性。 |
...
| `EMChatMessage.groupAckCount` | `EMChatMessage.groupReadReceiptCount` | 获取群聊消息的已读人数；5.0 为只读属性。 |
...
iOS SDK 5.0 将群组可见性、入群审批和成员邀请权限从 `EMGroupStyle` 单一枚举改为 `EMGroupConfigs` 中的独立属性。**该调整不提供兼容层，升级时需要修改建群和群组配置代码。**

### `EMGroupStyle` 与 `EMGroupConfigs` 对照

| 4.x `EMGroupStyle`（已删除） | 5.0 `EMGroupConfigs` 配置 |
| :--- | :--- |
...
| `NSString *ext` | 属性保持不变；5.0 默认值为空字符串 |
...
随密码登录下线，基于“用户 ID + 密码”的设备鉴权接口一并移除，5.0 保留基于“用户 ID + Token”的异步接口：
...
| `EMStreamChunk.sequenceNumber` | `EMStreamChunk.isComplete` | 判断流式消息分片状态。 | 5.0 不再公开分片序列号；使用只读属性判断流式消息是否完成。 |
...
| `EMClientDelegate` | `userAccountDidLoginFromOtherDevice:`、无参数的 `userAccountDidLoginFromOtherDevice` | `userAccountDidLoginFromOtherDeviceWithInfo:` | 当前账号在其他设备登录的通知；5.0 通过 `EMLoginExtensionInfo` 提供登录设备及扩展信息。 |
...
4. **消息已读状态不再允许应用直接修改**
   ...
   `EMChatMessage.isRead` 在 5.0 中为只读属性，`EMConversation` 的逐条和全量标记已读接口也已删除。业务应使用会话未读数清理接口；消息已读回执则通过独立的批量回执接口发送。
