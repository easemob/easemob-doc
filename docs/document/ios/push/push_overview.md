...
推送模板是离线推送的高级功能。使用前，你需要在 环信控制台的 **功能配置 > 基础功能 > 消息** 页面激活。**激活后，如需关闭推送模板功能，必须联系商务，因为该操作会删除推送模板相关的所有配置。**

推送模板的配置和使用，详见 [相关文档介绍](push_display.html#使用推送模板)。

## 多设备离线推送策略

多设备登录时，可在 环信控制台的 **证书管理** 页面配置推送策略，该策略配置对所有推送通道生效：
