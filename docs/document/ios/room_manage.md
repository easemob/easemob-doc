...
- **携带扩展信息加入**：需要在加入聊天室时传递自定义业务信息，或控制是否退出已加入的其他聊天室时，调用 `joinChatroom` 方法。
...
如果需要在加入聊天室时传递自定义业务信息，请调用 `joinChatroom` 方法。该方法还支持控制用户加入当前聊天室时，是否退出已加入的其他聊天室。
...
用户成功加入聊天室后，聊天室内的其他成员会收到 `userDidJoinChatroom` 回调。如果加入聊天室时传入了扩展信息，其他成员可以通过该回调获取。
