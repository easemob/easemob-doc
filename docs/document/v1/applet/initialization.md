...
若支持 SDK 注册，需登录[环信即时通讯云控制台](/document/v1/privatization/uc_configure.html)，选择 **应用概览** > **应用详情**，将 **应用设置**下的 **用户注册模式** 设置为 **开放注册**。
...
**用户 ID + token** 是更加安全的登录方式。token 可以通过调用 REST API 获取，详见 [环信用户 token 的获取](/document/v1/server-side/easemob_user_token.html)。
