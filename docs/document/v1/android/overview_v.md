...
开始前，请注册有效的环信即时通讯 IM 开发者账号并取得 App key，见 [环信即时通讯云管理后台](/document/v1/privatization/uc_configure.html)。
...
- 正式环境应使用服务器端调用 REST API 接口[注册用户](/document/v1/server-side/account_system.html#注册用户)。
  :::
...
手动登录时传入的用户 ID 必须为 String 类型，支持的字符集详见[用户注册的 RESTful 接口](/document/v1/server-side/account_system.html#注册用户)。
