...
[IM SDK] [新增 `EMChatManager#asyncFetchConversationsFromServer(int, int, EMValueCallBack)` 方法实现从服务端分页获取会话列表](message_retrieve.html#从服务器分页获取会话列表)。
...
- [IM SDK] [发送前回调](/document/v1/server-side/callback.html#_1、发送前回调)：发送失败时返回给 app 用户的错误描述中增加你自定义的错误信息（即 [响应体参数](/document/v1/server-side/callback.html#响应体参数) code 信息）。
