...
开始前，请注册有效的环信即时通讯 IM 开发者账号且获得 App key，见 [环信即时通讯云管理后台](/document/v1/privatization/uc_configure.html)。
...
登录[环信即时通讯云控制台](/document/v1/privatization/uc_configure.html)，选择 **运营服务** > **用户管理**，创建 IM 用户。

### REST API 注册

请参考 [注册用户](/document/v1/server-side/account_system.html#注册用户)。

### SDK 注册

若支持 SDK 注册，需登录[环信即时通讯云控制台](/document/v1/privatization/uc_configure.html)，选择 **应用概览** > **应用详情**，将 **设置**下的 **用户注册模式** 设置为 **开放注册**。
...
登录时传入的用户 ID 必须为 String 类型，支持的字符集详见[用户注册的 RESTful 接口](/document/v1/server-side/account_system.html#注册用户)。
...
2. **用户 ID + token** 是更加安全的登录方式。token 可以通过调用 REST API 获取，详见 [环信用户 token 的获取](/document/v1/privatization/easemob_user_token.html)。
