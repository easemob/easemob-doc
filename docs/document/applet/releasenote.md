...
-  加入聊天室 `joinChatRoom` 成功的回调新增 `info` 字段，包含如下信息，即用户加入聊天室后会收到如下信息：
   - 聊天室创建时间：`createTimestamp`，新增属性。
   - 是否开启全员禁言：`isAllMembersMuted`，新增属性。
   - 是否在白名单中：`isInAllowlist`，新增属性。
   - 当前聊天室成员数：`memberCount`，新增属性。
   - 成员禁言到期时间：`muteExpireTimestamp`，新增属性。
...
- [IM SDK] 修复 Uniapp SDK 4.36 版本运行到鸿蒙平台，无限重连的问题。
